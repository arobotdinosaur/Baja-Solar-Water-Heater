...
const float startupBufferTemperature = 5.0; //The roof must be this many degrees warmer than the water heater for the pump to kick in
const float shutdownBufferTemperature = 3.0;

const float cutoffTemperature = 50.0;
...
if (lastTemperature_1 < 5){
  pumpStatus = 0;
  digitalWrite(pumpPin,LOW);
  display.clearDisplay();
  display.setCursor(0, 0);
  display.print("Thermocouple 1 reading below 5 degrees celsius, pump shut off");
}
else if (lastTemperature_2 < 5){
  pumpStatus = 0;
  digitalWrite(pumpPin,LOW);
  display.clearDisplay();
  display.setCursor(0, 0);
  display.print("Thermocouple 2 reading below 5 degrees celsius, pump shut off");
}
...
modifiedPumpController();
}
}
// will deeletete     pumpStatus == 0 && lastTemperature_1 < targetTemp && lastTemperature_1 < (lastTemperature_2
void modifiedPumpController(){
  if (lastTemperature_1 < cutoffTemperature){
    digitalWrite(pumpPin,HIGH);
    pumpStatus = 1;
    ...
    if (lastTemperature_1>lastTemperature_2+shutdownBufferTemperature){
      ...
      pumpStatus = 0;
      display.setCursor(0, 10);
      display.print("Pump off for 1 hour because water was not getting heated");
      delay(3600000);
    }
  }
  else {
